...
class CPManager:
    """
    Manager for Prefill Context Parallelism and Decode Context Parallelism 
    (PCP & DCP, collectively referred to as CP) metadata and buffers.

    This manager encapsulates all CP-related buffers and logic so that the
    ModelRunner can access them via `self.cp_manager`.
    """
    ...
    def __init__(
        ...
        arange_np: Optional[np.ndarray] = None,
    ) -> None:
        ...
        max_buffer_num_tokens = self.max_num_tokens + 2 * self.pcp_size * self.max_num_reqs
    ...
    def update_tokens_for_pcp(
        ...
        2 * pcp_size chunks and ranks are assigned chunks in an interleaved
        ...
        Example:
        >>> Assume tokens = [1, 5, 8], pcp_size = 2. After _update_tokens_for_pcp.
        ...
        # DualChunkSwap requires alignment to a multiple of (2 * pcp_size).
        # We first pad each request's token count up to that multiple.
        num_padded_scheduled_tokens = np.ceil(
            num_scheduled_tokens / (2 * self.pcp_size)).astype(
                np.int32) * (2 * self.pcp_size)

        # PCP does not split decode requests. For decode requests, we instead
        # duplicate the scheduled tokens across the pcp_size ranks.
        num_padded_scheduled_tokens[:num_decode_reqs] = (
            num_scheduled_tokens[:num_decode_reqs] * self.pcp_size)
        ...
        def get_current_rank_positions(positions_start_loc: int | np.ndarray,
            ...
            - For tail chunks: start at positions_start_loc + (2*pcp_size- rank -
            1) * chunk_size.
        ...
        if num_decode_reqs > 0:
            positions[:num_decode_tokens] = self._get_cumsum_and_arange(
                num_scheduled_tokens[:num_decode_reqs])[1]
        ...
        idx_len = prefill_resotre_idx.shape[0]
        self.num_prefill_
        self.pcp_allgather_restore_idx_prefill.np[:idx_len] = (
            prefill_resotre_idx)
        self.pcp_allgather_resotre_idx_prefill.copy_to_gpu(idx_len)
    ...
    def get_logits_indices(self, cu_num_tokens: np.ndarray, num_reqs: int):
        return (torch.from_numpy(cu_num_tokens) * self.pcp_size -
    ...
    def get_discard_request_mask(
        ...
        return (num_computed_tokens_cpu[:num_reqs] +
                num_scheduled_tokens * self.pcp_size -
    ...
    def generate_cp_mtp_input(
        ...
        And we also pre-allocate mtp slot_mapping for mtpx with (p/d)cp.
        """
        ...
        self.query_lens_pcp_full.copy_to_gpu(num_reqs)
    ...
    def _get_cp_local_seq_lens(
        ...
        total_world_size = pcp_size * dcp_size
    ...
    def generate_cp_metadata(self, total_num_scheduled_tokens, query_lens,
        ...
        query_lens_new = self.query_lens_pcp_full.cpu[:num_reqs] \
            if self.pcp_size > 1 and self.speculative_config else query_lens
        ...
        cp_metadata = AscendContextParallelMetadata(
        ...
        if self.pcp_size > 1:
            ...
            cp_metadata.pcp_allgather_restore_idx = self.pcp_allgather_restore_idx.gpu[:
                                                                                     num_actual_tokens_pcp_padded]
            cp_metadata.pcp_allgather_restore_idx_prefill = self.pcp_allgather_restore_idx_prefill.gpu[:idx_len]
            cp_metadata.q_head_idx = q_head_idx
            cp_metadata.q_tail_idx = q_tail_idx
            cp_metadata.q_full_idx = q_full_idx
            cp_metadata.kv_with_q_head_nomask_idx = kv_with_q_head_nomask_idx
            cp_metadata.kv_with_q_head_mask_idx = kv_with_q_head_mask_idx
            cp_metadata.kv_with_q_tail_nomask_idx = kv_with_q_tail_nomask_idx
            cp_metadata.kv_with_q_tail_mask_idx = kv_with_q_tail_mask_idx
            ...
            cp_metadata.pcp_prefill_mask = pcp_prefill_mask
        return cp_metadata
