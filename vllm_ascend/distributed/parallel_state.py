...
def init_ascend_model_parallel(
    ...
    all_ranks = torch.arange(world_size).reshape(-1, global_dp_size * global_pcp_size * global_tp_size)
    ...
    if pd_head_ratio > 1 and get_current_vllm_config().kv_transfer_config.is_kv_producer:
        ...
        if num_head_replica <= 1:
            ...
            group_ranks = all_ranks.clone().view(
                global_dp_size * global_pcp_size, -1, num_head_replica
            ...
            group_ranks = group_ranks.unsqueeze(-1).view(
                global_dp_size * global_pcp_size, num_head_replica, -1, alltoall_group_size
