...
from vllm.distributed import get_tensor_model_parallel_world_size, get_tp_group, get_dcp_group, get_pcp_group
...
class AscendSFAImpl(MLAAttentionImpl):
    ...
    def gather_kv_cross_cp(self, kv_cache: torch.Tensor, block_tables: torch.Tensor | None =None) -> Tuple[torch.Tensor, torch.Tensor | None]:
        ...
        if block_tables is not None:
            block_tables = (block_tables.unsqueeze(-1) + (torch.arange(self.pcp_size * self.dcp_size) * block_num).view(1, 1, -1).to(block_tables)).reshape(block_tables.shape[0], -1)
