                ...
                if self.runner.pcp_manager.pcp_use_hybrid_attn:
                    ...
                    if pcp_padded_tokens_fla > 0:
                        hidden_states = nn.functional.pad(
                            hidden_states, pad=(0, 0, 0, pcp_padded_tokens_fla), mode="constant", value=0
                        )
                    hidden_states = get_pcp_group().all_gather(hidden_states.contiguous(), dim=0)
                    total_pcp_padding = self.runner.pcp_manager.total_pcp_padding_tokens_fla
                    restore_idx = self.runner.pcp_manager.pcp_enter_fa_restore_idx[
                        : hidden_states.shape[0] - total_pcp_padding
                    ]
                    hidden_states = torch.index_select(hidden_states, 0, restore_idx)
                else:
                    hidden_states = get_pcp_group().all_gather(hidden_states, 0)
                    restore_idx = self.runner.pcp_manager.pcp_allgather_restore_idx.gpu
                    hidden_states = torch.index_select(hidden_states, 0, restore_idx[: hidden_states.shape[0]])
