...
def layer_norm_gated_fwd_kernel(
    ...
    x_rows = rows // (X_N1 * X_N2) * X_S0 + (rows // X_N2 % X_N1) * X_S1 + rows % X_N2 * X_S2
    ...
    g_rows = rows // (G_N1 * G_N2) * G_S0 + (rows // G_N2 % G_N1) * G_S1 + rows % G_N2 * G_S2
    ...
    # Write output
    y_rows = rows // (Y_N1 * Y_N2) * Y_S0 + (rows // Y_N2 % Y_N1) * Y_S1 + rows % Y_N2 * Y_S2
...
def rms_norm_gated(
    ...
    if residual is None and not prenorm and x.shape[-1] <= 512 and 2 <= x.ndim <= 4 and 2 <= g.ndim <= 4:
        ...
        if rows:
            ...
            block_rows = 4 if rows <= small_batch_rows else 32
