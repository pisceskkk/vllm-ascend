...
def test_sfa_remap_sparse_indices_triton(
    dcp_size: int,
    rank: int,
    interleave_size: int,
    num_rows: int,
    ...
    actual = remap_sparse_indices_triton(indices, dcp_size, rank, interleave_size)
    torch.testing.assert_close(actual, expected, atol=0, rtol=0)


@pytest.mark.parametrize("top_k", [1, 7, 512, 2048, 8192])
@torch.inference_mode()
def test_sfa_remap_sparse_indices_triton_dynamic_top_k(top_k: int) -> None:
    indices = torch.arange(5 * top_k, dtype=torch.int32, device="npu").view(5, 1, top_k)
    indices[..., ::11] = -1

    expected = _reference_remap(indices, 16, 7, 128)
    actual = remap_sparse_indices_triton(indices, 16, 7, 128)
    torch.testing.assert_close(actual, expected, atol=0, rtol=0)


@torch.inference_mode()
def test_sfa_remap_sparse_indices_triton_large_integer_precision() -> None:
    indices = torch.tensor(
    ...
    actual = remap_sparse_indices_triton(indices, 2, 0, 128)
    torch.testing.assert_close(actual, expected, atol=0, rtol=0)
