...
@pytest.mark.parametrize("case", ["empty", "fp32", "weight_stride", "comm_limit", "decode_or_mixed"])
def test_fused_o_proj_runtime_fallback_preserves_token_shard(monkeypatch, rank, case):
    ...
    elif case == "decode_or_mixed":
        monkeypatch.setattr(linear_op, "is_forward_context_available", lambda: True)
        monkeypatch.setattr(linear_op, "get_forward_context", lambda: SimpleNamespace(is_pure_prefill=False))
