...
from tests.ut.models.test_kimi_attn_res_add_cpu import fused_reference, native_reference
...
def runtime(monkeypatch):
    ...
    def with_add(prefix, addend, blocks, projection, gamma, eps):
        combined = prefix + addend
        return native_reference(combined, blocks, projection, gamma, eps), combined
    ...
    namespace = {
        ...
        "_MRV2_MODEL": ContextVar("test_mrv2_model", default=None),
    }
    ...
    load_definitions(
        ...
        methods={
            ...
            "AscendKimiDecoderLayer": {
                "forward",
                "prepare_attn_residual",
                "forward_attn_residual",
                "_run_self_attn",
                "_run_mlp",
            },
        },
...
def test_pipeline_shards_match_unsplit_model(
    runtime, num_tokens, block_size, materialized, cuts, dtype, runner_version, tp=2
):
...
@pytest.mark.parametrize("materialized", [False, True])
def test_tp8_dspark_c1_c4_with_block_and_pp_boundaries(runtime, num_tokens, materialized):
    test_pipeline_shards_match_unsplit_model(
        runtime, num_tokens, 2, materialized, (0, 1, 3, 5), torch.bfloat16, "mrv2", tp=8
    )


@pytest.mark.parametrize("tokens", [1, 4])
@pytest.mark.parametrize("materialized", [False, True])
def test_93_layer_chain_uses_187_native_points_and_keeps_all_aux_values(runtime, monkeypatch, tokens, materialized):
    class WeightedNorm(nn.Module):
        def __init__(self, weight):
            super().__init__()
            self.weight = torch.tensor(weight, dtype=torch.bfloat16)
            self.variance_epsilon = 1e-5

        def forward(self, value):
            normalized = value.float() * torch.rsqrt(value.float().square().mean(-1, keepdim=True) + 1e-5)
            return (normalized * self.weight.float()).to(value.dtype)

    namespace, context = runtime
    context.tp = 1
    context.pp = SimpleNamespace(is_first_rank=True, is_last_rank=True)
    model = make_model(namespace, context, 0, 93, 12, False, materialized)
    for layer in model.layers:
        layer.input_layernorm = WeightedNorm([0.875, 1.125, 0.5])
        layer.post_attention_layernorm = WeightedNorm([1.25, 0.75, 1.5])
    model.aux_hidden_state_layers = (0, 1, 11, 12, 13, 92, 93)
    inputs = (torch.arange(tokens * 3).reshape(tokens, 3) / 10).bfloat16()
    # Independent unfused model order, including the BF16 add at every tail.
    raw = inputs.clone()
    bank = torch.empty(tokens, 8, 3, dtype=raw.dtype)
    aux = [] if materialized else [raw]
    for index, layer in enumerate(model.layers):
        valid = (index + 11) // 12
        pre = (
            native_reference(
                raw, bank[:, :valid], layer.self_attention_res_proj.weight, layer.self_attention_res_norm.weight, 1e-5
            )
            if valid
            else raw
        )
        if materialized and index in model.aux_hidden_state_layers:
            aux.append(pre)
        attention_output = layer.input_layernorm(pre)
        if index % 12 == 0:
            bank[:, index // 12].copy_(raw)
            prefix = attention_output
            valid += 1
        else:
            prefix = raw + attention_output
        post = native_reference(prefix, bank[:, :valid], layer.mlp_res_proj.weight, layer.mlp_res_norm.weight, 1e-5)
        raw = prefix + (layer.post_attention_layernorm(post) * 0.25 + 0.1)
        if not materialized and index + 1 in model.aux_hidden_state_layers:
            aux.append(raw)
    expected = native_reference(raw, bank, model.output_attn_res_proj.weight, model.output_attn_res_norm.weight, 1e-5)
    if materialized:
        aux.append(expected)
    ...
    def record(*args, **kwargs):
        ...
        return fused_reference(*args, **kwargs)

    monkeypatch.setattr(native_reference, "fused", record)
    actual, actual_aux = model(None, torch.arange(tokens), None, inputs_embeds=inputs)
    torch.testing.assert_close(actual, expected, rtol=0, atol=0)
    assert len(actual_aux) == len(aux)
    for observed, reference in zip(actual_aux, aux):
        torch.testing.assert_close(observed, reference, rtol=0, atol=0)
    assert len(calls) == 187  # pre 93 + post 93 + model output 1
    assert sum(args[1] is not None for args, _ in calls) == 93 + 85
    assert sum(len(args) > 9 and args[9] >= 0 for args, _ in calls) == 8
    assert all(kwargs.get("mix", True) for _, kwargs in calls)
