...
class TestMtpProposer:
    ...
    def test_generate_token_ids(self, mock_cpu_gpu_buffer):
        ...
        mock_deps.runner.cp_manager = MagicMock()
        mock_deps.runner.cp_manager.input_ids_pcp_full = CpuGpuBuffer(
            32,
            dtype=torch.int32,
            pin_memory=False,
            device='cpu',
        )
        mock_deps.runner.cp_manager.input_ids_pcp_full.cpu = \
            torch.arange(32, dtype=torch.int32)
        mock_deps.runner.cp_manager.query_start_loc_pcp_full = CpuGpuBuffer(
            5,
            dtype=torch.int32,
            pin_memory=False,
            device='cpu',
        )
        mock_deps.runner.cp_manager.query_start_loc_pcp_full.cpu = \
